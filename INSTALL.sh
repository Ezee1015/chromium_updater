...
if [ $KernelOS == "Darwin" ]; then 
  ...
  while [ "$(crontab -l | grep ".chromium/AutoUpdate.sh")" != "* * * * 1 bash $HOME/.chromium/AutoUpdate.sh >> /dev/null" ]; do
    clear
    echo ""
    ...
    clear
    echo "Please add '* * * * 1 bash /Users/$(whoami)/.chromium/AutoUpdate.sh >> /dev/null' Then press control+'o' and Enter and control+'x' without the double quotes. And now you can close the terminal. Press Enter"
    echo ""
    read PAUSE
...
if [ $KernelOS == "Linux" ]; then
  ...
  sudo echo "* * * * 1 bash $HOME/.chromium/AutoUpdate.sh >> /dev/null" >> /tmp/chron
